...
module.exports = {
    name: 'givecredits',
    description: '[Dev Only] Give Credits to a user',
    devOnly: true,
    execute(message, args) {
        ...
        if(args.length === 1) {
            ...
            .setDescription(`Please tell me how many credits i should give ${touser.tag}`)
            .setColor(16750848)
            channel.send(prompt).then(() => {
                ...
                channel.awaitMessages(filter, {
                    ...
                    if(amount != 0) {
                        creditmanager.addCredits(touser.id, amount);
                        const doneembed = new Discord.MessageEmbed()
                        .setTitle('Credits added')
                        .setDescription(`${amount} credits where given to ${touser.tag}`)
                        .setColor(1376000);
                        channel.send(doneembed);
                        return;
                    }
                    const notembed = new Discord.MessageEmbed()
                        .setTitle('No credits added')
